...
const renderVariant = (variantCode) => {
  const match = variantCode.match(/<(\w+)([^>]*)>(.*?)<\/\1>/);
  if (match) {
    ...
    const props = {};
    const propsMatches = propsString.match(/(\w+)="([^"]*)"/g);
    if (propsMatches) {
      propsMatches.forEach(prop => {
    ...
    return (
      <Element canvas is={Component} {...props}>
        {children}
      </Element>
    );
  }
