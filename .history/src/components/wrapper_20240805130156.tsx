...
export const Wrapper = ({ children, style = {} }: WrapperProps) => {
  ...
  const togglePreviewMode = () => {
    ...
    actions.setOptions((options) => options.enabled = !isPreviewMode);
  };
  ...
  return (
    <div className="w-full h-full flex justify-center" style={style}>
      <div className={`${wrapperWidth} flex flex-col h-full border rounded-sm`}>
        ...
        <div
          className={`${wrapperWidth} flex-1 bg-white rounded-b-lg mx-auto transition-all duration-300 relative`}
          ref={(ref) => {
            if (ref && !isPreviewMode) {
              connect(drag(ref));
            }
          }}
        >
