...
import { renderComponents } from './renderComponents'; // Make sure this path is correct
...
export const ControlPanel = () => {
  ...
  useEffect(() => {
    if (active && active !== 'ROOT') {
      ...
      setVariants(['<Button>Button 2</Button>']);
    } else {
      setVariants([]);
    }
  }, [active]);
        ...
        <div className="p-4">
          ...
          {variants.map((variant, index) => (
            <div key={index} className="mb-4 border p-2 rounded">
              <div className="mb-2" style={{ height: '100px', overflow: 'auto' }}>
                <ReactIframe
                  ref={iframeRef}
                  title={`variant-${index}`}
