...
export const ResizableComponent = ({ width = 'auto', height = 'auto', children }) => {
  ...
  const renderDraggableChild = (child, index) => {
    return (
      <Draggable
        key={index}
        bounds="parent"
        onStart={(e) => e.stopPropagation()} // Prevent dragging the parent
      >
        <div style={{ position: 'absolute', cursor: 'move' }}>
          {React.cloneElement(child, {
            ...
            style: {
              ...child.props.style,
              position: 'static', // Reset position to allow dragging
            },
          })}
        </div>
      </Draggable>
    ...
    >
      {containerSize.width > 0 && containerSize.height > 0 && (
        ...
        >
          ...
          >
            {React.Children.map(children, renderDraggableChild)}
          </div>
        </ResizableBox>
      )}
    </div>
