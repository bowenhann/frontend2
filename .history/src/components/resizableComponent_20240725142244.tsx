...
export const ResizableComponent = ({ width = 'auto', height = 'auto', children }) => {
  ...
  const { enabled, resolvedNodes } = useEditor((state, query) => ({
    enabled: state.options.enabled,
    resolvedNodes: query.getNodes()
  }));
  ...
  const CanvasChildren = React.useMemo(() => {
    return React.Children.map(children, (child, index) => {
      ...
      if (!componentName) {
        console.warn(`Component at index ${index} does not have a name. It may not be properly resolved.`);
        return child;
      }
      return (
        <Element 
