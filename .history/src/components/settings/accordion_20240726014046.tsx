...
export const AccordionSettings = () => {
  ...
  const addItem = () => {
    setProp((props: any) => ({
      ...
      items: [
        ...(props.items || []),
        { triggerText: 'New Item', contentText: 'New Content' }
      ]
    }));
  };
  ...
  const updateItem = (index: number, field: 'triggerText' | 'contentText', value: string) => {
    setProp((props: any) => {
        ...
        {props.items?.map((item: any, index: number) => (
          <div key={index} className="space-y-2 border p-2 rounded">
            <Input
              ...
              onChange={(e) => updateItem(index, 'triggerText', e.target.value)}
              placeholder="Trigger text"
            />
            <Input
              value={item.contentText}
              onChange={(e) => updateItem(index, 'contentText', e.target.value)}
