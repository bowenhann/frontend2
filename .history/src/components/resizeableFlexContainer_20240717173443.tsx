...
export const ResizableContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const childrenWithSeparators = React.Children.toArray(children).reduce((acc: React.ReactNode[], child, index, array) => {
    if (index === array.length - 1) {
      return [...acc, React.cloneElement(child as React.ReactElement, { width: itemWidths[index] || `${100 / array.length}%` })];
    }
    return [
      ...acc,
      React.cloneElement(child as React.ReactElement, { width: itemWidths[index] || `${100 / array.length}%` }),
...
export const ResizableItem: React.FC<{ children: React.ReactNode; width: string }> = ({ children, width }) => {
  ...
  return (
    <div
      ...
      style={{
        flex: `0 0 ${width}`,
        width: width, // Add explicit width
        padding: '10px',
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: '#ddd',
        zIndex: 1000,
        alignSelf: 'stretch', // Ensure the separator stretches vertically
      }}
    />
  );
};
