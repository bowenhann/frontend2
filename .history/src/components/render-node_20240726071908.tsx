...
import React, { useEffect, useRef, useCallback, cloneElement } from 'react';
// import ReactDOM from 'react-dom';
// import { Move, ArrowUp, Trash2 } from 'lucide-react';
// import { Button } from '/ui/button';
import { generateComponentCode } from '@/lib/codeGenerator'; // 确保路径正确
...
export const RenderNode = ({ render }: { render: React.ReactNode }) => {
  ...
  return <div onClick={handleClick}>{render}</>;

  // 使用 cloneElement 为渲染的内容添加点击事件
  // const renderedWithClick = React.isValidElement(render) 
  //   ? cloneElement(render, { onClick: handleClick })
  //   : render;

  // return <>{renderedWithClick}</>;
};
