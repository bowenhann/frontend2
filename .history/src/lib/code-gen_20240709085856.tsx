...
export const getOutputCode = (nodes: Nodes) => {
  ...
  const processNode = (nodeId: string, level: number): string => {
    ...
    // 处理子节点
    const childrenCode = node.data.nodes
      .filter((childId, index, self) => self.indexOf(childId) === index) // 移除重复的直接子节点
      .map(childId => processNode(childId, level + 1))
      .join('\n');
