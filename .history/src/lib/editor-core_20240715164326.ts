...
type EditorContextType = {
  ...
  addComponent: (parentId: string | null, component: ComponentData) => string;
...
export const EditorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addComponent = (parentId: string | null, component: ComponentData): string => {
  ...
  const removeComponent = (id: string) => {
    setComponents(prev => {
      ...
      Object.keys(rest).forEach(key => {
        if (rest[key].children?.includes(id)) {
          rest[key] = {
            ...rest[key],
            children: rest[key].children!.filter(childId => childId !== id)
          };
        }
      });
      return rest;
...
export const useNode = (id: string) => {
  ...
  return {
    ...
    updateProps: (newProps: Record<string, any>) => updateComponent(id, { props: { ...components[id].props, ...newProps } })
  };
};