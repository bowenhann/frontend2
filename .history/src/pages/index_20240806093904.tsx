 ...
 useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const dynamicContentId = Object.keys(currentState).find(
          nodeId => currentState[nodeId].data.displayName === 'Dynamic Content'
        );

        if (!dynamicContentId) {
          console.error('DynamicContent node not found. Current state:', currentState);
          throw new Error('Could not find DynamicContent node');
        }

        console.log('Found DynamicContent node:', dynamicContentId);

        const newContentString = buttonStrings[stage];
        const newContentNode = {
          type: 'ResizableComponent',  // 显式指定组件类型
          props: {
            width: '100%',
            height: 'auto',
            children: newContentString
          }
        };

        console.log(`Updating content, adding Stage ${stage + 1}`);
        console.log('New content node:', newContentNode);
        
        try {
          actions.add(newContentNode, dynamicContentId);
          console.log('Successfully added new node');
          setStage(stage + 1);
        } catch (addError) {
          console.error('Error adding new node:', addError);
          console.log('Node being added:', newContentNode);
          console.log('Parent node ID:', dynamicContentId);
        }
