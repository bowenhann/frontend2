...
function renderComponents(componentsString: string) {
  ...
  console.log('Input string:', componentsString);
    ...
    if (componentName) {
      ...
      if (attributes) {
        ...
        while ((attributeMatch = attributeRegex.exec(attributes))) {
          ...
          const [, name, jsExpression, doubleQuotedValue, singleQuotedValue] = attributeMatch
          if (name === 'is') {
            Component = componentMap[jsExpression.trim()]
            console.log('Component set to:', jsExpression.trim());
          } else if (name === 'className') {
            props[name] = `${props[name] || ''} ${doubleQuotedValue || singleQuotedValue}`.trim()
          } else {
            props[name] = jsExpression || doubleQuotedValue || singleQuotedValue
          }
        }
      }
      ...
      if (Component) {
        console.log('Creating element for:', Component.name || 'Anonymous');
        try {
          const element = React.createElement(Component, { key: components.length, ...props })
        ...
        console.warn('Component not found in componentMap:', componentName);
      }
    } else if (textContent) {
      console.log('Adding text content:', textContent.trim());
