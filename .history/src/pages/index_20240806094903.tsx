...
const PlaceholderComponent = ({children}) => <div>{children}</div>;
...
const ContentUpdater = () => {
  ...
  const buttonStrings = [
    ...
    const timer = setInterval(() => {
      if (stage >= contentStages.length - 1) {
        clearInterval(timer);
        return;
      }

      const dynamicContentId = query.getNodes()['dynamic-content'].id;
      if (!dynamicContentId) {
        console.error('Dynamic content element not found');
        return;
      }

      const newContent = contentStages[stage];
      try {
        actions.add(
          { 
            displayName: newContent.type,
            props: newContent.props,
            custom: {},
            isCanvas: false,
          },
          dynamicContentId
        );
        console.log(`Successfully added new element: ${newContent.type}`);
        setStage(stage + 1);
      } catch (error) {
