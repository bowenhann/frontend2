...
// 优化的StageContent组件
const StageContent = React.memo(({ buttonString, stageId }) => {
  const {
    connectors: { connect, drag }
  } = useNode()

  const ButtonComponent = useMemo(() => renderComponents(buttonString), [buttonString])

  return (
    <Element 
      id={`stage_element_${stageId}`} 
      is={Container} 
      canvas 
      ref={(ref) => connect(drag(ref))}
    >
...
// 优化的NewContent组件
const NewContent = React.memo(({ children }) => {
  const {
    connectors: { connect, drag }
  } = useNode()
...
const StageManager = () => {
  ...
  const addStage = useCallback(() => {
    if (stages.length < buttonStrings.length) {
      const newStage = {
        id: `Stage_${stages.length}_${Date.now()}`,
        content: buttonStrings[stages.length]
      }
      setStages(prevStages => [...prevStages, newStage])
    }
  }, [stages, buttonStrings])
  ...
  return (
    <NewContent>
      {stages.map(stage => (
        <StageContent key={stage.id} stageId={stage.id} buttonString={stage.content} />
      ))}
    </NewContent>
  )
}
...
const App = () => {
  return (
    <div className="p-4">
      ...
      >
        <Frame>
          <Element is={StageManager} canvas />
        </Frame>
      </Editor>
    </div>
  )
