...
const DraggableElement = ({ is: Is, ...props }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
...
const LeftPanel = ({ onDragStart }) => {
  ...
  const handleDragStart = (e, id) => {
    e.dataTransfer.setData('node', id);
    onDragStart();
  };
...
const RightPanel = ({ onDrop, canDrop }) => {
  const { actions, query } = useEditor();

  const handleDrop = (e) => {
    const nodeId = e.dataTransfer.getData('node');
    if (nodeId === 'text') {
      actions.add(Text, { text: 'Dragged Text' });
    } else if (nodeId === 'container') {
  ...
  return (
    <div 
      ...
      style={{ 
        height: '100%', 
        padding: '20px',
        background: canDrop ? '#e6ffe6' : 'white',
        transition: 'background 0.3s'
      }}
    >
      <h2>Right Panel</h2>
      <Editor resolver={{ Text, Container, DraggableElement }}>
        <Frame>
          <Element is={Container} canvas>
            {/* Right panel content will be added here when elements are dropped */}
          </Element>
        </Frame>
      </Editor>
    </div>
...
const App = () => {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <h1>Craft.js with Cross-Panel Drag and Drop</h1>
      <PanelGroup direction="horizontal" style={{ flex: 1 }}>
        <Panel defaultSize={50} minSize={30}>
          <LeftPanel onDragStart={() => setCanDrop(true)} />
        </Panel>
        <PanelResizeHandle style={{ width: '10px', background: '#ddd', cursor: 'col-resize' }} />
        <Panel minSize={30}>
          <RightPanel onDrop={() => setCanDrop(false)} canDrop={canDrop} />
        </Panel>
      </PanelGroup>
    </div>
  );
