		...
		switch (type) {
			...
			case 'button':
				if (buttonString) {
					...
					return (
						<Element id={`button_${id}`} is={Canvas} canvas={true}>
							{ButtonComponent}
						</Element>
					)
				}
...
const Container = ({ children, className }) => {
	...
	return (
		<div
			ref={(ref) => connect(drag(ref))}
			className={`bg-white-100 p-4 min-h-[100px] m-2 border border-dashed border-green-300 ${className}`}
		>
			{children}
		</div>
	)
     ...
     <NodeCardFooter>
       <NodeButton>Footer button</NodeButton>
        ...
        `
				...
				console.log(
					'Updating content, adding NewContent (Text and Button) to DynamicContent'
				)
		...
		<div className="p-4">
			...
			>
				<Frame>
					...
					<Element is={DynamicContent} canvas></Element>
					</Element>
				</Frame>
				<ContentUpdater />
			</Editor>
