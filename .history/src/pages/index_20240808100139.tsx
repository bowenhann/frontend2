...
const ContentUpdater = () => {
 ...
 useEffect(() => {
    let newContentNodeId = null;

    const addOrUpdateNewContent = () => {
      const existingNewContent = query.node('NEW_CONTENT').get();
      if (!existingNewContent) {
        ...
        newContentNodeId = actions.add(node, 'ROOT');
        console.log('Added new NewContent node with ID:', newContentNodeId);
      } else {
        newContentNodeId = 'NEW_CONTENT';
