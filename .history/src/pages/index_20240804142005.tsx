...
const DynamicAdder = () => {
  ...
  const addDynamicText = () => {
    const newTextNode = {
      ...
      props: { text: `Dynamic Text ${Math.random().toString(36).substr(2, 5)}` },
    };
    actions.add(newTextNode, id);
  };

  const addDynamicButton = () => {
    const newButtonNode = {
      type: { resolvedName: 'Button' },
      props: { text: `Dynamic Button ${Math.random().toString(36).substr(2, 5)}` },
    };
    actions.add(newButtonNode, id);
  };
...
Container.craft = {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const newContent = 'This content was dynamically loaded after 2 seconds!';
      setContent(newContent);

      // 更新 Craft.js 中的节点
      const nodeId = query.node(id).get().data.nodes[0];
  ...
  return (
    <div className="bg-yellow-100 p-2 m-2 rounded">
      <Element id="dynamicText" is={Text} text={content} />
    </div>
  );
};
...
export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Craft.js with Dynamic Content Demo</h1>
      <Editor resolver={{ Text, Button, Container, DynamicContent }}>
        <Frame>
          <Element is={Container} canvas>
            <Element is={Text} text="Drag me around" />
            <Element is={Button} text="Draggable Button" />
            <Element is={DynamicContent} />
          </Element>
        </Frame>
      </Editor>
    </div>
