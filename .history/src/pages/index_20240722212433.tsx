...
function renderComponents(componentsString) {
	const regex = /<(\w+)(?:\s+is=(?:{([^}]+)}))?(?:\s+([^>]+))?>([\s\S]*?)<\/\1>|([^<]+)/gs;
	const components = [];

	let match;
	while ((match = regex.exec(componentsString))) {
			const [, componentName, isComponent, attributes, children, textContent] = match;

			if (componentName) {
					let Component = componentMap[componentName];

					if (componentName === 'Element' && isComponent) {
							Component = Element;
					}
					...
					if (Component) {
							...
							if (attributes) {
									...
									while ((attributeMatch = attributeRegex.exec(attributes))) {
											const [, name, jsValue, stringValue] = attributeMatch;
											if (name === 'className') {
													props[name] = `${props[name] || ''} ${stringValue}`.trim();
											} else if (jsValue) {
													props[name] = eval(jsValue); // Be cautious with eval
											} else {
							...
							if (componentName === 'Element') {
									components.push(
											<Component key={components.length} {...props}>
													{props.children}
											</Component>
									);
							} else {
...
{/* <Element is={ResizableComponent} width='30%' height='auto' canvas>
  <NodeCalendar className="p-4"></NodeCalendar>
</Element>
<Element is={ResizableComponent} width='20%' height='auto' canvas>
  <NodeButton className="bg-blue-500 text-white px-4 py-2 rounded">Button 1</NodeButton>
</Element>
<Element is={ResizableComponent} width='20%' height='auto' canvas>
  <NodeButton>Button 2</NodeButton>
...
<Element is={ResizableComponent} width='50%' height='auto' canvas>
  <NodeCard className="p-6 m-2">
    <NodeCardHeader>
      <NodeCardTitle className="bg-blue-500 text-white px-4 py-2 rounded">Card Title</NodeCardTitle>
      <NodeCardDescription className="bg-blue-500 text-white px-4 py-2 rounded">Card Description</NodeCardDescription>
