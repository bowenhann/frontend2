...
const Container = ({ background, padding }) => {
  ...
  return (
    <div 
      ...
      style={{ background, padding, margin: '5px', border: '1px solid black' }}
    >
      <h2>Container</h2>
      <button onClick={() => {
        const { data: { props } } = node(id).get();
        add(
          createNode(React.createElement(Container, { ...props, background: randomColor() }))
        );
      }}>
        Add Copy
      </button>
...
const App = () => {
  return (
    <div style={{ margin: '0 auto', width: '800px' }}>
      <h1>My App</h1>
      <Frame>
        <Element is={Container} background="#999" padding={20} />
      </Frame>
    </div>
  )
}
