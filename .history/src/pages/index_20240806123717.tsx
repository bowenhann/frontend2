  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        console.log('ROOT node nodes property:', safeStringify(rootNode.data.nodes));
        ...
        actions.addNodeTree(newNodeTree, 'ROOT');

        // Update the ROOT's nodes array by appending the new node
        const updatedRootNode = query.node('ROOT').get();
        if (updatedRootNode && Array.isArray(updatedRootNode.data.nodes)) {
          const updatedNodes = [...updatedRootNode.data.nodes, newNodeId];
          console.log('Updating ROOT nodes:', updatedNodes);
          actions.setCustom('ROOT', {
            ...updatedRootNode.data.custom,
            nodes: updatedNodes
          });
        } else {
          console.error('Failed to update ROOT node');
        }

        // Check if the new node was actually added
        setTimeout(() => {
          const updatedNodes = query.getNodes();
          console.log('Updated nodes after adding:', safeStringify(updatedNodes));
          if (updatedNodes[newNodeId]) {
            console.log('New node successfully added');
            console.log('Updated ROOT node structure:', safeStringify(query.node('ROOT').get()));
          } else {
