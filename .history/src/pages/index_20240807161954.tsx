  ...
  useEffect(() => {
    ...
    const createInitialContainer = () => {
      const containerTree = query.parseReactElement(
        <Element
          ...
          stage={0}
        >
          <Element canvas is="div" id="dynamic_content_inner" />
        </Element>
      ).toNodeTree();

      actions.addNodeTree(containerTree, 'ROOT');
    };
    ...
    const timer = setInterval(() => {
      ...
      try {
        ...
        const updatedContentTree = query.parseReactElement(
          <Element
            canvas
            is={NewContent}
            buttonStrings={buttonStrings}
          ...
          >
            <Element canvas is="div">
              {renderComponents(buttonStrings.slice(0, stage + 2).join('\n'))}
            </Element>
          </Element>
        ).toNodeTree();

        actions.replaceNodes(['dynamic_content_container'], () => updatedContentTree);
