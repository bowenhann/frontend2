...
const generateUniqueId = (prefix = 'element') => `${prefix}_${elementCounter++}`;
...
const replace = (domNode) => {
  if (domNode.type === 'tag') {
    if (domNode.name.toLowerCase() === 'element') {
      ...
      const elementId = id || generateUniqueId(is);
      
      return (
        <Element id={elementId} {...props} component={Component}>
          {domNode.children && domNode.children.map(child => {
            if (child.type === 'tag') {
              return parse(child.data || '', { replace });
