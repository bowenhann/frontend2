  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const findNewContentNode = (nodeId) => {
          const node = parsedCurrentState[nodeId]
          if (node.type.resolvedName === 'NewContent') {
            return nodeId
          }
          for (const childId of node.nodes || []) {
            const result = findNewContentNode(childId)
            if (result) return result
          }
          return null
        }

        const newContentId = findNewContentNode('ROOT')

        if (!newContentId) {
          console.error('Could not find NewContent node')
          return
        }

        console.log(`Updating content, setting Stage to ${stage + 1}`)

        // Update the existing NewContent node's props
