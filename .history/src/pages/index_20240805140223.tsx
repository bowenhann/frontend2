...
const IncrementalRenderer = ({ buttonStrings }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      if (renderedComponents.length < buttonStrings.length) {
        setRenderedComponents(prev => [
          ...prev,
          renderComponents(buttonStrings[prev.length])
        ])
      } else {
        clearInterval(timer)
      }
...
IncrementalRenderer.craft = {
  displayName: 'Incremental Renderer',
  props: {
    buttonStrings: []
  }
}
...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    ...
    const incrementalRendererId = `IncrementalRenderer_${Date.now()}`
    ...
    const newState = {
      ...
      [dynamicContentId]: {
        ...
        nodes: [
          ...
          incrementalRendererId
        ]
      },
      [incrementalRendererId]: {
        type: { resolvedName: 'IncrementalRenderer' },
        isCanvas: false,
        props: {
