...
// const Container = ({ background, padding, children }) => {
...
//     const newText = query.createNode(React.createElement(Text, {
//       text: `New text component ${count}`,
//       fontSize: 16
//     }));
//     actions.add(newText, 'ROOT');
//     setCount(prev => prev + 1);
//   };

//   return (
//     <div ref={connect}>
//       <Container background="#f0f0f0" padding={10}>
//         <Text text="Welcome to the dynamic content area" fontSize={18} />
//         <Button text="Add Text Component" onClick={addTextComponent} />
//       </Container>
//     </div>
//   );
// };
...
//       <Editor resolver={{ Container, Button, Text, DynamicContent }}>
//         <Frame>
...
const ResizableComponent = ({ width, height, children }) => {
  ...
  return (
    <div 
      ref={(dom) => connect(drag(dom))} 
      style={{ width, height }}
      className="border border-gray-300 p-4 m-2 resize overflow-auto"
    >
      {children}
    </div>
  );
...
const NodeButton = ({ children, className }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <button
      ref={(dom) => connect(drag(dom))}
...
const NodeCalendar = ({ className }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(dom) => connect(drag(dom))} className={`border p-4 ${className}`}>
...
const StreamingContent = () => {
  ...
  const componentString = `
    ...
    <ResizableComponent width="20%" height="50%">
      <NodeButton className="bg-red-500 text-white">Button 1</NodeButton>
    </ResizableComponent>
    <ResizableComponent width="20%" height="50%">
      <NodeButton>Button 2</NodeButton>
    </ResizableComponent>
  ...
  const componentArray = componentString.trim().split('</ResizableComponent>');

  useEffect(() => {
    const interval = setInterval(() => {
      if (currentIndex < componentArray.length) {
        const componentStr = componentArray[currentIndex] + '</ResizableComponent>';
        const component = query.parseReactElement(componentStr).toNodeTree();
        actions.add(component, 'ROOT');
  ...
  }, [currentIndex, actions, query]);

  return null;
};

// Craft 配置
ResizableComponent.craft = { displayName: 'Resizable Component' };
NodeButton.craft = { displayName: 'Button' };
NodeCalendar.craft = { displayName: 'Calendar' };
NodeCard.craft = { displayName: 'Card' };
StreamingContent.craft = { displayName: 'Streaming Content' };

// 主组件
export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Craft.js Streaming Render Demo</h1>
      <Editor
        resolver={{
          ResizableComponent,
          NodeButton,
      ...
      >
        <Frame>
          <Element is={StreamingContent} canvas />
        </Frame>
      </Editor>
    </div>
  );
