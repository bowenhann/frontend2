...
const Container = ({ children }) => {
  ...
  const handleCopy = () => {
    const { id } = node((n) => n.data.type === Container).get();
    const { data: { props } } = node(id).get();
    add(createNode(Element, Container, props));
  };

  return (
    <div 
      ref={(dom) => connect(drag(dom))}
      className="border-2 border-dashed border-gray-300 p-4 m-2 bg-white"
    >
      <h2 className="text-lg font-bold mb-2">Draggable Container</h2>
      <p className="mb-4">This is a container that can be dragged and duplicated.</p>
      {children}
      <button 
        onClick={handleCopy}
      ...
      <Editor resolver={{ Container }}>
        <Frame>
          <Element is={Container} canvas>
            <p>This is the initial content of the container.</p>
          </Element>
        </Frame>
      </Editor>
    </div>
