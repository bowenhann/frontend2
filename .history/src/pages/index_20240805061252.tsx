...
// Reusable wrapper to make components draggable
const Draggable = ({ children }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(ref) => connect(drag(ref))} className="p-2 m-2 border border-dashed border-gray-300 cursor-move">
      {children}
    </div>
  );
};
...
const NewContent = ({ content, type }) => {
  const renderContent = () => {
    switch (type) {
      case 'text':
        return <div className="bg-pink-200 p-3 rounded-md shadow-sm">{content}</div>;
      case 'button':
        return <NodeButton className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition-colors">{content}</NodeButton>;
      default:
        return content; // For any other custom components
    }
  };
...
// Updated DynamicContent component
const DynamicContent = ({ children }) => {
  return (
    <Draggable>
      <div className="bg-yellow-100 p-4 rounded-lg shadow-md min-h-[150px]">
        <h3 className="text-lg font-semibold mb-2">Dynamic Content Container</h3>
        {children}
      </div>
    </Draggable>
  );
...
const Container = ({ children }) => {
  return (
    <Draggable>
      <div className="bg-green-100 p-6 rounded-lg shadow-md min-h-[200px]">
        {children}
      </div>
    </Draggable>
  );
      ...
      try {
        ...
        const newState = {
          ...
          [newContentTextId]: {
            ...
            isCanvas: false,
            props: { content: 'New Text Content (Drag me!)', type: 'text' },
          ...
          [newContentButtonId]: {
            type: { resolvedName: 'NewContent' },
            isCanvas: false,
            props: { content: 'New Button', type: 'button' },
...
const App = () => {
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center">Draggable Components Demo</h1>
      <Editor resolver={{ DynamicContent, TextBox, Container, NewContent, NodeButton }}>
        <Frame>
          <Element is={Container} canvas>
            ...
            <Element is={TextBox} canvas>
              <TextBox text="Drag me around!" />
            </Element>
            <Element is={Container} canvas>
              <TextBox text="Nested Text Box" />
            </Element>
          </Element>
        </Frame>
        <ContentUpdater />
