...
const NewContent = memo(({ buttonStrings, stage }) => {
  const {
    connectors: { connect, drag }
  } = useNode()

  const renderContent = useCallback(() => {
    return buttonStrings.slice(0, stage + 1).map((str, index) => {
      const ButtonComponent = renderComponents(str)
      return (
        <animated.div
          key={index}
          style={useSpring({
        ...
        >
          <Element id={`dynamic_content_${index}`} is={Container} canvas={true}>
            {ButtonComponent}
          </Element>
        </animated.div>
      )
    ...
    const updateContent = async () => {
      try {
        ...
        const dynamicContentId = Object.keys(parsedCurrentState).find(
          key => parsedCurrentState[key].type.resolvedName === 'DynamicContent'
        )
        ...
        await actions.deserialize(newState)
      } catch (error) {
