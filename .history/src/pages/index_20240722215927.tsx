...
function renderComponents(componentsString) {
  ...
  while ((match = regex.exec(componentsString))) {
    ...
    if (Component) {
      ...
      if (name === 'Element') {
        // Extract width and height from props
        const { width, height, ...restProps } = props
        
        // Create ResizableComponent with correct props
        const resizableComponent = React.createElement(
          ResizableComponent,
          { width, height },
          restProps.children
        )

        // Create Element wrapping ResizableComponent
        components.push(
          React.createElement(
            Element,
            { key: components.length, ...restProps, is: ResizableComponent },
            resizableComponent
          )
        )
      } else {
