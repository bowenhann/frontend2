  ...
  useEffect(() => {
    const addComponentsSafely = async () => {
      try {
        ...
        for (let i = 0; i < predefinedComponents.length; i++) {
          const component = predefinedComponents[i]
          const nodeId = `dynamic-${i}`
          
          console.log(`Attempting to add component ${nodeId}:`, component)
          ...
          try {
            const node = await new Promise((resolve, reject) => {
              ...
              const addedNode = query.node(nodeId).get()
              if (addedNode) {
                resolve(addedNode)
              } else {
                reject(new Error(`Failed to add node ${nodeId}`))
              }
            })
            
            console.log(`Successfully added component ${nodeId}:`, node)
          } catch (componentError) {
            console.error(`Error adding component ${nodeId}:`, componentError)
          }
        }
