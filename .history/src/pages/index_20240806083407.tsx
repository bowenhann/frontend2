  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        // 修改开始: 使用 addNodeTree 替代 deserialize
        const currentState = query.serialize();
        const parsedCurrentState = JSON.parse(currentState);

        const findDynamicContainer = (nodeId) => {
          const node = parsedCurrentState[nodeId];
          if (node.type.resolvedName === 'DynamicContainer') {
            return nodeId;
          }
          for (const childId of node.nodes || []) {
            const result = findDynamicContainer(childId);
            if (result) return result;
          }
          return null;
        };

        let dynamicContainerId = findDynamicContainer('ROOT');

        if (!dynamicContainerId) {
          // 如果 DynamicContainer 不存在，创建它
          dynamicContainerId = `DynamicContainer_${Date.now()}`;
          actions.addNodeTree({
            [dynamicContainerId]: {
              type: { resolvedName: 'DynamicContainer' },
              isCanvas: true,
              props: {},
        ...
        const newContentId = `IncrementalContent_${Date.now()}`;

        // 创建新的节点树
        const newNodeTree = {
          [newContentId]: {
            type: { resolvedName: 'IncrementalContent' },
            isCanvas: false,
            props: {
              buttonString: buttonStrings[stage]
            },
            displayName: `Incremental Content ${stage + 1}`,
            custom: {},
            hidden: false,
            nodes: [],
            linkedNodes: {},
        ...
        // 使用 addNodeTree 添加新的内容
        actions.addNodeTree(newNodeTree, dynamicContainerId);
