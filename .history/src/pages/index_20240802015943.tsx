...
const Container = ({ children }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '10px', margin: '5px', border: '1px solid #ccc', minHeight: '50px' }}>
      {children}
    </div>
  );
};
...
const ResizableContainer = ({ children }) => {
  ...
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ width: '100%', height: '400px', border: '2px solid #666' }}>
      <PanelGroup direction="horizontal">
        ...
        <PanelResizeHandle style={{ width: '10px', background: '#ccc' }} />
        <Panel minSize={20}>
          <div style={{ height: '100%', overflow: 'auto' }}>
            {children[1]}
          </div>
...
const App = () => {
  return (
    <div style={{ margin: '50px' }}>
      ...
      <Editor resolver={{ Text, Container, ResizableContainer }}>
        <Frame>
          <Element is={ResizableContainer} canvas>
            <Element is={Container} canvas>
              <Element is={Text} text="Left panel nested text 1" />
              <Element is={Text} text="Left panel nested text 2" />
              <Element is={Container} canvas>
                <Element is={Text} text="Nested container in left panel" />
              </Element>
            </Element>
            <Element is={Container} canvas>
              <Element is={Text} text="Right panel nested text 1" />
              <Element is={Text} text="Right panel nested text 2" />
              <Element is={Container} canvas>
                <Element is={Text} text="Nested container in right panel" />
              </Element>
            </Element>
          </Element>
        </Frame>
