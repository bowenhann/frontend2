...
import { Editor, Frame, Element, useEditor, NodeElement } from '@craftjs/core';
...
const ContentUpdater = () => {
  const { actions, query, createNodeFromJSXElement } = useEditor();

  useEffect(() => {
    const timer = setTimeout(() => {
      console.log('Current nodes:', JSON.stringify(query.getNodes(), null, 2));

      // Create a new Text node
      const newElement = createNodeFromJSXElement(<Text text="New Text" />);

      if (newElement) {
        // Add the new node to the ROOT
        actions.add(newElement, 'ROOT');
    ...
    return () => clearTimeout(timer);
  }, [actions, query, createNodeFromJSXElement]);
