...
import { useEditor, useNode, Element, Editor, } from '@craftjs/core'
...
const DynamicContent = () => {
  ...
  const addNewComponent = () => {
    const newNode = query.createNode(
      React.createElement(SimpleDiv, { background: 'lightblue', padding: 10 }, 'New Component')
    )
    add(newNode, id)
  }
...
const Container = () => {
  ...
  return (
    <div ref={(dom) => connect(drag(dom))}>
      <h2>Container</h2>
      <Element id="dynamic-content" is={DynamicContent} canvas />
    </div>
  )
}
