...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const dynamicContentElement = query.getNodes()['dynamic-content'];
      ...
      const newElement = {
        type: {
          resolvedName: 'NodeButton'  // 确保这匹配你的 resolver 中的名称
        },
        props: contentStages[stage].props,
        parent: 'dynamic-content'
      };

      try {
        actions.add(newElement, 'dynamic-content');
        console.log('Successfully added new element');
        setStage(stage + 1);
      } catch (error) {
  ...
  }, [actions, query, stage, contentStages]);
