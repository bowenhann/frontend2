...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const rootNodeId = Object.keys(nodes).find(id => nodes[id].data.type.resolvedName === 'Container');
      ...
      if (rootNodeId) {
        ...
        actions.addNodeTree(newNodeTree, rootNodeId);

        // Update the parent's nodes array
        const updatedNodes = [...nodes[rootNodeId].data.nodes, newNodeId];
        actions.setCustom(rootNodeId, (node) => {
          node.nodes = updatedNodes;
        });

        setCount(count + 1);
        console.log('Nodes after addition:', query.getNodes());
      }
    }, 2000);
