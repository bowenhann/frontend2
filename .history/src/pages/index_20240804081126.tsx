...
function simulateStringGeneration(fullString, callback) {
  ...
  const intervalId = setInterval(() => {
    if (currentLineIndex < lines.length) {
      ...
      if (currentCharIndex < currentLine.length) {
        callback(lines.slice(0, currentLineIndex).join('\n') + 
  ...
  }, 50) // Adjust the interval (in milliseconds) to control the speed of rendering
...
export default function Home() {
  ...
  useEffect(() => {
    const cleanup = simulateStringGeneration(originalButtonString, setButtonString)
    return cleanup
  }, [])
