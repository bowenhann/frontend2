...
// 								</NodeButton>
// 						</ResizableComponent>
// 						<Element is={DynamicContent} canvas>               {showNewContent ? <NewContent /> : 'Initial Content'}
...
const InitialContent = () => {
  ...
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '10px', margin: '10px', border: '1px solid blue' }}>
      Initial Content
    </div>
  );
};
...
const NewContent = () => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '10px', margin: '10px', border: '1px solid green' }}>
      New Content
    </div>
  );
};
...
const DynamicContent = () => {
  ...
  return (
    <div style={{ padding: '10px', margin: '10px', border: '1px solid black' }}>
      <Element
        id="dynamicChild"
        is={({ isNewContent }) => isNewContent ? NewContent : InitialContent}
        isNewContent={false}
      />
    </div>
  );
...
const App = () => {
  return (
    <div>
      <h1>Dynamic Content Demo with Draggable Subcomponents</h1>
      <Editor resolver={{ DynamicContent, InitialContent, NewContent }}>
        <Frame>
