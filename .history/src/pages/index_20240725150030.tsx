...
function renderComponents(componentsString) {
  ...
  let match;
  while ((match = regex.exec(componentsString))) {
    const [, openTagName, openAttributes, children, selfClosingTagName, selfClosingAttributes, textContent] = match;
    const componentName = openTagName || selfClosingTagName;
    ...
    if (componentName) {
      ...
      if (Component) {
        if (componentName === 'Element') {
          components.push(React.createElement(Element, { ...props, component: Component }));
        } else {
