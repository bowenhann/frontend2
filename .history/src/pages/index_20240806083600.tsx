  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        // 查找或创建 DynamicContainer
        let dynamicContainerId = query.getNodes((node) => node.data.type === DynamicContainer)[0]?.id;

        if (!dynamicContainerId) {
          // 如果 DynamicContainer 不存在，创建它
          dynamicContainerId = actions.add(
            <Element is={DynamicContainer} canvas>
              {/* IncrementalContent 将在这里动态添加 */}
            </Element>,
            'ROOT'
          );
        }

        // 创建新的 IncrementalContent
        const newContentId = actions.add(
          <Element 
            is={IncrementalContent}
            buttonString={buttonStrings[stage]}
          />,
          dynamicContainerId
        );

        console.log(`Adding Stage ${stage + 1}, new content ID: ${newContentId}`);
        setStage(stage + 1);
      } catch (error) {
