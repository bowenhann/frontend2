...
const NewContent = ({ buttonStrings, stage }) => {
  ...
  const renderContent = () => {
    ...
    const ButtonComponent = renderComponents(combinedString);
		return (
      <Element id="dynamic_content_container" is={Container} canvas>
        {ButtonComponent}
      </Element>
    );
  };
...
const ContentUpdater = () => {
  ...
  const [newContentNodeId, setNewContentNodeId] = useState(null);
    ...
    },
    {
      ...
      props: {
        ...
        height: "50%",
         children: (
          <Element canvas is={NodeCard} className="p-6 m-2">
            <Element canvas is={NodeCardHeader}>
              <Element canvas is={NodeCardTitle} className="bg-blue-500 text-white px-4 py-2 rounded">
                Card Title
              </Element>
              <Element canvas is={NodeCardDescription} className="bg-blue-500 text-white px-4 py-2 rounded">
                Card Description
              </Element>
            </Element>
            <Element canvas is={NodeCardContent}></Element>
            <Element canvas is={NodeCardFooter}>
              <Element canvas is={NodeButton}>Footer button</Element>
            </Element>
          </Element>
        )
      }
  ...
  useEffect(() => {
    const addComponent = () => {
      if (stage >= componentStages.length) {
      ...
      try {
        const currentStage = componentStages[stage];
        
        // Create a node tree with Element wrapper to ensure canvas and draggable properties
        const nodeTree = query.parseReactElement(
          <Element
            canvas
            is={currentStage.type}
            {...currentStage.props}
          />
        ).toNodeTree();
        
        // Add the new node tree to the ROOT
        actions.addNodeTree(nodeTree, 'ROOT');

        // Log the updated structure after a short delay
        setTimeout(() => {
    ...
    const timer = setInterval(addComponent, 2000);

    return () => clearInterval(timer);
  }, [actions, query, stage, componentStages]);
...
// const H2 = ({ children }) => {
//   const { connectors: { connect, drag } } = useNode();
//   return <h2 ref={ref => connect(drag(ref))}>{children}</h2>;
// };
