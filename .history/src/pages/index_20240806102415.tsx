...
const ContentUpdater = () => {
  ...
  const buttonStrings = [
    ...
    const timer = setInterval(() => {
      try {
        console.log('Timer tick - Current stage:', stage);

        if (stage >= buttonStrings.length - 1) {
          console.log('Reached final stage, clearing interval');
          clearInterval(timer);
          return;
        }

        const currentState = query.getNodes();
        console.log('Current state:', currentState);

        const findDynamicContent = (nodeId) => {
          console.log('Checking node:', nodeId);
          const node = currentState[nodeId];
          if (!node) {
            console.log('Node not found:', nodeId);
            return null;
          }
          console.log('Node type:', node.data.type);
          if (node.data.type === 'DynamicContent') {
            console.log('Found DynamicContent node:', nodeId);
            return nodeId;
          }
        ...
        let dynamicContentId = findDynamicContent('ROOT');
        console.log('DynamicContent node ID:', dynamicContentId);

        if (!dynamicContentId) {
          console.log('DynamicContent node not found. Attempting to add to ROOT.');
          const rootNode = currentState['ROOT'];
          if (!rootNode) {
            throw new Error('ROOT node not found');
          }
          
          const newDynamicContentId = `DynamicContent_${Date.now()}`;
          const newDynamicContent = {
            id: newDynamicContentId,
          ...
          console.log('Adding new DynamicContent node:', newDynamicContent);
          actions.add(newDynamicContent, 'ROOT');
          
          dynamicContentId = newDynamicContentId;
        }

        const newContentId = `NewContent_${Date.now()}`;
        console.log('Creating new content with ID:', newContentId);
        ...
        actions.add(newNode, dynamicContentId);

        console.log(`Updated content, added Stage ${stage + 1}`);
        setStage(stage + 1);
      } catch (error) {
