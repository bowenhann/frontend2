...
const NewContent = ({ buttonStrings, stage }) => {
  ...
  const renderContent = () => {
    return buttonStrings.map((stringContent, index) => {
      const ButtonComponent = renderComponents(stringContent);
      return (
        <Element 
          key={index} 
          canvas 
          is={Container} 
          className={index === stage ? "border-2 border-blue-500" : ""}
        >
          {ButtonComponent}
        </Element>
      );
...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    // Add the initial NewContent component if it doesn't exist
    const addInitialContent = () => {
      const existingNode = query.node('dynamic_content_container').get();
      if (!existingNode) {
    ...
    const timer = setInterval(() => {
      ...
      try {
        // Update the existing NewContent component
        actions.setProp('dynamic_content_container', (props) => {
          return {
            ...props,
            buttonStrings: buttonStrings.slice(0, stage + 2),
            stage: stage + 1
          };
        });
