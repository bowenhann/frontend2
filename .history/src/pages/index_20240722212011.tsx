...
function renderComponents(componentsString) {
  const regex = /<(Element|[A-Z]\w*)(\s[^>]*)?>([\s\S]*?)<\/\1>|([^<]+)/g;
  const components = [];
    ...
    if (componentName) {
      ...
      if (attributes) {
        ...
        while ((attributeMatch = attributeRegex.exec(attributes))) {
          ...
          if (name === 'is') {
            Component = eval(jsExpression); // 注意：在生产环境中应避免使用 eval
          } else {
            props[name] = jsExpression ? eval(jsExpression) : (doubleQuotedValue || singleQuotedValue);
          }
        }
      }
