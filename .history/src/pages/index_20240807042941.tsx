...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      console.log('Current nodes:', JSON.stringify(currentNodes, null, 2));

      // Try to find the root container
      let rootNodeId = 'ROOT'; // Default to 'ROOT'
      if (!currentNodes['ROOT']) {
        rootNodeId = Object.keys(currentNodes).find(id => 
          currentNodes[id].data && 
          currentNodes[id].data.type && 
      ...
      const rootNode = currentNodes[rootNodeId];
      ...
      const updatedNodes = {
        ...
        [rootNodeId]: {
          ...
          data: {
            ...rootNode.data,
            nodes: [...(rootNode.data.nodes || []), newNodeId]
          }
        },
        [newNodeId]: {
          data: {
      ...
      console.log('Updated nodes:', JSON.stringify(updatedNodes, null, 2));

      actions.deserialize(updatedNodes);

      console.log('Nodes after deserialize:', JSON.stringify(query.getNodes(), null, 2));
    }, 2000);
