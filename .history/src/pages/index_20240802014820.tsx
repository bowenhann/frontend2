...
const Text = ({ text }) => {
  ...
  return <p ref={(ref) => connect(drag(ref))} style={{ margin: '10px', padding: '10px', background: '#fff', border: '1px solid #ddd' }}>{text}</p>;
};

Text.craft = {
  props: { text: 'Draggable Text' },
  rules: { canDrag: () => true },
};

const StaticText = () => {
  return <p style={{ margin: '10px', padding: '10px', background: '#eee' }}>This is static text in the Left Panel</p>;
};

const PanelContent = ({ children }) => {
  const { connectors: { connect } } = useNode();
  return (
    <div ref={connect} style={{ padding: '10px', height: '100%', overflow: 'auto' }}>
      {children}
    </div>
  );
};
  ...
  return (
    <div style={{ width: '100%', height: '400px' }}>
      <PanelGroup direction="horizontal">
        <Panel minSize={20}>
          <Element is={PanelContent} id="left-panel-content" canvas>
            <StaticText />
            <Element is={Text} id="text-1" text="Draggable Item 1" />
            <Element is={Text} id="text-2" text="Draggable Item 2" />
          </Element>
        </Panel>
        <PanelResizeHandle />
        <Panel minSize={20}>
          <Element is={PanelContent} id="right-panel-content" canvas>
            <Element is={Text} id="text-3" text="Draggable Item 3" />
            <Element is={Text} id="text-4" text="Draggable Item 4" />
          </Element>
        </Panel>
      </PanelGroup>
    </div>
...
const App = () => {
  return (
    <div style={{ margin: '50px' }}>
      ...
      <Editor resolver={{ Text, ResizableContainer, PanelContent }}>
        <Frame>
          <Element is={ResizableContainer} id="resizable-container" canvas>
            {/* The ResizableContainer now manages its own children */}
          </Element>
        </Frame>
      </Editor>
    </div>
