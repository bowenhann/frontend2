...
const Text = ({ text }) => {
  ...
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '10px', margin: '10px', backgroundColor: 'lightgray' }}>
      {text}
    </div>
  );
};
...
const ResizableContainer = ({ children }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ width: '100%', height: '300px' }}>
      <PanelGroup direction="horizontal">
        <Panel minSize={20}>
          <div style={{ padding: '10px', height: '100%', backgroundColor: 'lightblue' }}>
            {children[0]}
          </div>
        </Panel>
        <PanelResizeHandle style={{ width: '10px', backgroundColor: 'gray' }} />
        <Panel minSize={20}>
          <div style={{ padding: '10px', height: '100%', backgroundColor: 'lightgreen' }}>
            {children[1]}
          </div>
        </Panel>
      </PanelGroup>
...
ResizableContainer.craft = {
  displayName: 'ResizableContainer',
};

const App = () => {
  return (
    <div>
      <h1>Craft.js + react-resizable-panels Demo</h1>
      <Editor resolver={{ Text, ResizableContainer }}>
        <Frame>
          <Element is={ResizableContainer} canvas>
            <Element is={Text} text="Left panel (draggable)" />
            <Element is={Text} text="Right panel (draggable)" />
          </Element>
        </Frame>
      </Editor>
    </div>
