...
const Draggable = ({ children }) => {
  ...
  return (
    <div ref={(ref) => connect(drag(ref))} className="p-2 m-1 border border-dashed border-gray-300">
      {children}
    </div>
  );
};
...
const NewContent = ({ content, type, className, buttonProps, id }) => {
  const renderContent = () => {
    switch (type) {
      case 'text':
        return <div className={className}>{content}</div>;
      case 'button':
        return <NodeButton {...buttonProps} />;
      default:
        return <div className={className}>{content}</div>;
    }
  };
...
// DynamicContent component (now with Tailwind classes)
const DynamicContent = ({ children, className }) => {
  return (
      <div className={`bg-yellow-100 p-4 min-h-[100px] ${className}`}>
...
// TextBox and Container components (updated with Tailwind classes)
...
const Container = ({ children, className }) => {
  return (
      <div className={`bg-green-100 p-4 min-h-[100px] ${className}`}>
        {children}
      </div>
  );
};
...
const App = () => {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Draggable Components Demo</h1>
      <Editor resolver={{ DynamicContent, TextBox, Container, NewContent, NodeButton, ResizableComponent }}>
        <Frame>
          <Element is={Container} canvas className="border-2 border-gray-300 p-4">
            <Element is={DynamicContent} canvas className="mb-4">
              {/* Initially empty */}
            </Element>
            <Element is={TextBox} canvas>
              <TextBox text="Drag me around!" className="font-bold" />
            </Element>
            <Element is={Container} canvas className="mt-4">
