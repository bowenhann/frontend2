...
const NewContent = React.memo(({ buttonStrings, stage }) => {
  const {
    connectors: { connect, drag }
  } = useNode()
...
const Container = React.memo(({ children }) => {
  ...
  return (
    <div
      ref={(ref) => connect(drag(ref))}
      className="bg-gray-100 p-4 min-h-[100px] m-2 border border-dashed border-green-300"
    >
      {children}
    </div>
  )
...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(updateContent, 2000)

    return () => clearTimeout(timer)
  }, [stage, buttonStrings, updateContent])
