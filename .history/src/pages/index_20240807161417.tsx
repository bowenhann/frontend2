...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      try {
        // Find the existing NewContent container
        const newContentNode = query.node('ROOT').descendants().find(node => 
          node.data.displayName === 'Dynamic Content Container'
        );

        if (!newContentNode) {
          // If the container doesn't exist, create it
          const newContentTree = query.parseReactElement(
            <Element
              canvas
              is={NewContent}
          ...
          actions.addNodeTree(newContentTree, 'ROOT');
        } else {
          // If the container exists, add new content to it
          const newElement = query.parseReactElement(
            renderComponents(buttonStrings[stage + 1])
          ).toNodeTree();

          actions.addNodeTree(newElement, newContentNode.id);

          // Update the stage prop of the NewContent component
          actions.setProp(newContentNode.id, props => {
            props.stage = stage + 1;
          });
        }
