  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data.type === 'resize' && iframeRef.current) {
        ...
        const resizableElement = iframe.contentDocument.querySelector('.resizable-component');
        if (resizableElement) {
          resizableElement.style.width = `${event.data.size.width}px`;
          resizableElement.style.height = `${event.data.size.height}px`;
        }
      }
    };
