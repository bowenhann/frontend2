  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        ...
        // Get the ID of the DynamicContent node
        ...
        const newState = {
          ROOT: {
            ...parsedCurrentState.ROOT,
            type: { resolvedName: 'DynamicContent' }, // Keep it as DynamicContent
            props: { content: 'New Content' }, // Update the content here
          },
          [dynamicContentId]: {
        ...
        console.log('New state to be applied:', newState);

        actions.deserialize(newState);
        
        console.log('State after deserialization:', query.serialize());
      } catch (error) {
  ...
  return (
    <div>
      ...
      <Editor resolver={{ DynamicContent }}>
        <Frame>
          <Element is={DynamicContent} canvas>
            <DynamicContent content="Initial Content" />
          </Element>
        </Frame>
        <ContentUpdater />
      </Editor>
