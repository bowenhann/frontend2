...
const DynamicContentContainer = ({ children }) => {
  const {
    connectors: { connect, drag }
  } = useNode()
    ...
    const timer = setInterval(() => {
      try {
        ...
        const findDynamicContent = (nodeId) => {
          ...
          if (node.type.resolvedName === 'DynamicContentContainer') {
            return nodeId
          }
        ...
        if (!dynamicContentId) {
          throw new Error('Could not find DynamicContentContainer node')
        }
			...
			>
				<div className="flex flex-1 relative overflow-hidden">
					...
					<Viewport>
						<Frame>
							<Element is={Wrapper}>
								<Element is={DynamicContentContainer}></Element>
							</Element>
						</Frame>
					</Viewport>
					<ControlPanel />
