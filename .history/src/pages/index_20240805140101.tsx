...
const IncrementalContent = ({ buttonString }) => {
  const {
    connectors: { connect, drag }
  } = useNode()
...
IncrementalContent.craft = {
  displayName: 'Incremental Content',
  props: {
    buttonString: ''
  }
}
...
DynamicContentWrapper.craft = {
  displayName: 'Dynamic Content Wrapper'
}
    ...
    const timer = setInterval(() => {
      try {
        ...
        const findDynamicContent = (nodeId) => {
          ...
          if (node.type.resolvedName === 'DynamicContentWrapper') {
            return nodeId
          }
        ...
        if (!dynamicContentId) {
          throw new Error('Could not find DynamicContentWrapper node')
        }
        ...
        const newState = {
          ...
          [newContentId]: {
            ...
            displayName: `Incremental Content Stage ${stage + 1}`,
            custom: {},
            hidden: false,
            nodes: [],
            linkedNodes: {},
...
const App = () => {
  return (
    <div className="p-4">
      ...
      >
        <Frame>
          <Element is={DynamicContentWrapper} canvas>
            {/* IncrementalContent 组件将在这里动态添加 */}
          </Element>
        </Frame>
        <ContentUpdater />
      </Editor>
