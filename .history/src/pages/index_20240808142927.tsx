  ...
  const [newContentNodeId, setNewContentNodeId] = useState(null);
    ...
    },
    {
      ...
      props: {
        width: "32%",
        height: "50%",
        children: (<NodeCard className="p-6 m-2" canvas>
					<NodeCardHeader>
						<NodeCardTitle className="bg-blue-500 text-white px-4 py-2 rounded">
							Card Title
						</NodeCardTitle>
    ...
    if (React.isValidElement(node)) {
      return (
        <Element
          ...
          {...node.props}
        >
          {React.Children.map(node.props.children, child => wrapWithElement(child))}
        </Element>
      );
    }
    return node;
  ...
  useEffect(() => {
    const addComponent = () => {
      ...
      try {
        ...
        // Wrap the entire component tree with Element
        const wrappedComponent = wrapWithElement(
          <currentStage.type {...currentStage.props} />
        );
        
        // Create a node tree
        const nodeTree = query.parseReactElement(wrappedComponent).toNodeTree();
