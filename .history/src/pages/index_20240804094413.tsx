...
const Container = ({ background }) => {
  ...
  return (
    <div 
      ...
      style={{ background, padding: '10px', margin: '10px', border: '1px solid black' }}
    >
      <h2>Container</h2>
      <Button />
    </div>
...
const Button = () => {
  ...
  return (
    <button
      ref={connect}
      onClick={() => {
        const newNode = query.createNode(Element.create(Container));
        actions.add(newNode, query.node('ROOT').get().id);
      }}
    >
      Add Container
    </button>
...
const App = () => {
  return (
    <div>
      <h1>Craft.js Container Demo</h1>
      <Editor resolver={{ Container, Button }}>
        <Frame>
          <Element is={Container} canvas>
            <Element is={Container} background="#e0e0e0" />
          </Element>
        </Frame>
      </Editor>
    </div>
