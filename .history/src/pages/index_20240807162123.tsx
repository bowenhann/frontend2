...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    // Create a single container on mount
    const containerNodeTree = query.parseReactElement(
      <Element
        canvas
        is={NewContent}
        ...
        stage={0}
      >
        <Element canvas is="div" id="dynamic_content_container">
          {/* Content will be updated here dynamically */}
        </Element>
      </Element>
    ).toNodeTree();

    actions.addNodeTree(containerNodeTree, 'ROOT');
    ...
    const timer = setInterval(() => {
      ...
      try {
        ...
        const dynamicContent = renderComponents(buttonStrings.slice(0, stage + 2).join('\n'));
        const updatedNodeTree = query.parseReactElement(dynamicContent).toNodeTree();

        actions.replaceNodes(['dynamic_content_container'], updatedNodeTree);
