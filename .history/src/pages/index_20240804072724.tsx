...
import  StreamingComponentRenderer  from '@/lib/streamingRenderer'
...
export default function Home() {
    ...
    return () => {
      return new Promise<string | null>((resolve) => {
        if (index < buttonString2.length) {
          // Simulate chunk size (adjust as needed)
          const chunkSize = Math.min(10, buttonString2.length - index);
          const chunk = buttonString2.slice(index, index + chunkSize);
          index += chunkSize;
          setTimeout(() => resolve(chunk), 100); // Simulate network delay
        } else {
    ...
    //         > */}
		...
		// </section>
		<section className="w-full min-h-screen flex flex-col">
      <StreamingComponentRenderer getStreamData={getStreamData()} />
			</section>
	)
}
