...
const ResizableComponent = ({ width, height, children }) => {
  ...
  return (
    <div 
      ref={(dom) => connect(drag(dom))} 
      style={{ width, height }}
      className="border border-gray-300 p-2 m-2 resize overflow-auto"
    >
      {children}
    </div>
  );
...
const NodeCalendar = ({ className }) => {
  ...
  return (
    <div
      ref={(dom) => connect(drag(dom))}
      className={`${className} border border-gray-300 p-2 m-1`}
    >
...
// 模拟解析字符串并返回组件的函数
const parseStringToComponent = (str) => {
  const components = [];
  const lines = str.split('\n');
  lines.forEach((line, index) => {
    if (line.includes('<ResizableComponent')) {
      const widthMatch = line.match(/width="([^"]+)"/);
      const heightMatch = line.match(/height="([^"]+)"/);
      const width = widthMatch ? widthMatch[1] : '100%';
      const height = heightMatch ? heightMatch[1] : '100px';
      components.push(
        <Element
          canvas
          ...
          key={`resizable-${index}`}
        >
          {line.includes('<NodeCalendar') && (
            <Element is={NodeCalendar} className="p-4" />
          )}
          {line.includes('<NodeButton') && (
            <Element 
              is={NodeButton} 
              className={line.includes('bg-red-500') ? 'bg-red-500 text-white' : ''}
            >
              Button
            </Element>
          )}
...
<ResizableComponent width="20%" height="50%">
  ...
  useEffect(() => {
    const parsedComponents = parseStringToComponent(buttonString);
  ...
  useEffect(() => {
    if (currentIndex < components.length) {
      const timer = setTimeout(() => {
        actions.add(components[currentIndex], 'ROOT');
        setCurrentIndex(prevIndex => prevIndex + 1);
      }, 1000); // 每秒添加一个组件

      return () => clearTimeout(timer);
    }
  }, [currentIndex, components, actions]);
...
NodeButton.craft = { displayName: 'Node Button' };
NodeCalendar.craft = { displayName: 'Node Calendar' };
DynamicContent.craft = { displayName: 'Dynamic Content' };

export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Gradual Rendering Demo</h1>
      <Editor
        resolver={{
          ResizableComponent,
          NodeButton,
