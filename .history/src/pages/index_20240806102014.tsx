  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const findDynamicContent = (state) => {
          const queue = Object.keys(state);
          while (queue.length > 0) {
            const nodeId = queue.shift();
            console.log('Checking node:', nodeId);
            const node = state[nodeId];
            if (node.data.type === 'DynamicContent' || node.data.displayName === 'DynamicContent') {
              console.log('Found DynamicContent node:', nodeId);
              return nodeId;
            }
            if (node.data.nodes) {
              queue.push(...node.data.nodes);
            }
          }
          return null;
        };

        const dynamicContentId = findDynamicContent(currentState);
