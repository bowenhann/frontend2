...
<ResizableComponent width="20%" height="50%">
  <LoggableNodeButton className="bg-red-500 text-white px-4 py-2 rounded">
    Button 1
  </LoggableNodeButton>
</ResizableComponent>
<ResizableComponent width="20%" height="50%">
...
const withCodeLogger = (WrappedComponent) => {
  return (props) => {
    const { id, related } = useNode();
    const { nodes } = related;

    const handleClick = (e) => {
      e.stopPropagation(); // Prevent event bubbling
      const componentCode = getOutputCode({ [id]: nodes[id] });
      console.log('Component Code:', componentCode.output);
    };
...
// Wrap your existing components with the withCodeLogger HOC
export const LoggableNodeButton = withCodeLogger(NodeButton);
export const LoggableNodeCard = withCodeLogger(NodeCard);
