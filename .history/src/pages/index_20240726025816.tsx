...
function renderComponents(componentsString, isNested = false) {
  const regex = /<(\w+)(\s[^>]*)?>(.*?)<\/\1>|<(\w+)(\s[^>]*)?\/>|([^<]+)/gs;
  const components = [];
  ...
  while ((match = regex.exec(componentsString))) {
    ...
    if (componentName || selfClosingName) {
      ...
      if (children) {
        props.children = renderComponents(children, true);
      }
      ...
      if (Component) {
        if (Component === ResizableComponent) {
          ...
          if (isNested) {
            components.push(resizableElement);
          } else {
            components.push(
              <Element
                key={`wrapper-${components.length}`}
                is={ResizableComponent}
                canvas
              >
                {resizableElement}
