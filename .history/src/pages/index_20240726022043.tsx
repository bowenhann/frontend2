...
function renderComponents(componentsString) {
  ...
  while ((match = regex.exec(componentsString))) {
    ...
    if (componentName || selfClosingName) {
      ...
      let Component = componentMap[name] || name;  // 如果在 componentMap 中找不到，就使用原始名称
      const props = {};
      ...
      if (attributesString) {
        ...
        while ((attributeMatch = attributeRegex.exec(attributesString))) {
          ...
          if (name === 'is') {
            Component = componentMap[jsValue || doubleQuotedValue || singleQuotedValue] || (jsValue || doubleQuotedValue || singleQuotedValue);
          } else if (name === 'className') {
      ...
      if (typeof Component === 'string') {
        // 处理原生 HTML 元素
        components.push(React.createElement(Component, { key: components.length, ...props }, props.children));
      } else if (Component === ResizableComponent) {
        components.push(
          <Element
            key={components.length}
            is={ResizableComponent}
