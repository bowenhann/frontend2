...
// Modify Text and Button components to accept and use a `custom` prop
const Text = ({ text, custom }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div 
      ref={ref => connect(drag(ref))}
      className={`bg-blue-100 p-2 m-2 rounded ${custom ? 'border-2 border-red-500' : ''}`}
    >
      {text}
    </div>
  );
...
const Button = ({ text, custom }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <button 
      ref={ref => connect(drag(ref))}
      className={`bg-green-500 text-white p-2 m-2 rounded ${custom ? 'border-2 border-red-500' : ''}`}
    >
      {text}
    </button>
  );
...
const DynamicAdder = () => {
  ...
  const addDynamicText = () => {
    actions.add(<Text text="Dynamically Added Text" custom />, 'ROOT');
  };

  const addDynamicButton = () => {
    actions.add(<Button text="Dynamic Button" custom />, 'ROOT');
  };

  return (
    <div className="bg-yellow-100 p-4 m-2 rounded">
      <h3 className="font-bold mb-2">Dynamic Content Adder</h3>
      <button onClick={addDynamicText} className="bg-blue-500 text-white p-2 m-1 rounded">
        Add Text
      </button>
      <button onClick={addDynamicButton} className="bg-green-500 text-white p-2 m-1 rounded">
        Add Button
      </button>
    </div>
  );
...
DynamicAdder.craft = {
  displayName: 'Dynamic Adder',
};

export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Craft.js Drag and Drop with Draggable Dynamic Content Demo</h1>
      <Editor resolver={{ Text, Button, Container, DynamicAdder }}>
        <Frame>
          <Element is={Container} canvas>
            <Element is={Text} text="Drag me around" />
            <Element is={Button} text="Draggable Button" />
            <Element is={DynamicAdder} />
          </Element>
        </Frame>
      </Editor>
    </div>
