...
function renderComponents(componentsString: string) {
  const regex = /<Element\s+([^>]*)>([\s\S]*?)<\/Element>|([^<]+)/g;
  const components = [];

  let match;
  while ((match = regex.exec(componentsString))) {
    const [, attributes, children, textContent] = match;
    ...
    if (attributes) {
      ...
      if (trimmedContent) {
        components.push(trimmedContent);
      }
    }
  }

  console.log('Rendered components:', components); // 调试输出
  return components;
}
