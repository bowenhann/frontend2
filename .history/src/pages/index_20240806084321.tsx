  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const findNodeByType = (nodeId, targetType) => {
          const node = parsedCurrentState[nodeId]
          if (node.type.resolvedName === targetType) {
            return nodeId
          }
          for (const childId of node.nodes || []) {
            const result = findNodeByType(childId, targetType)
            if (result) return result
          }
          return null
        }

        const dynamicContentId = findNodeByType('ROOT', 'DynamicContent')
        if (!dynamicContentId) {
          console.error('Could not find DynamicContent node')
          return
        }

        let newContentId = findNodeByType('ROOT', 'NewContent')

        console.log(`Updating content, setting Stage to ${stage + 1}`)

        if (!newContentId) {
          // Create NewContent node if it doesn't exist
          const newNode = {
          ...
          console.log('Created new NewContent node')
        } else {
          // Update existing NewContent node
          actions.setProp(newContentId, (props) => ({
            ...props,
            stage: stage + 1,
            buttonStrings: buttonStrings
          }))
          console.log('Updated existing NewContent node')
        }
