...
// Split the original string into meaningful chunks
const chunks = originalButtonString.split('</ResizableComponent>').map(chunk => chunk + '</ResizableComponent>')
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (chunkIndex < chunks.length) {
        setButtonString(prev => prev + chunks[chunkIndex])
        chunkIndex++
      } else {
        clearInterval(interval)
      }
    }, 1000) // Adjust this value to control the speed of generation
