  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const findOrCreateDynamicContent = () => {
          const rootNode = currentState['ROOT'];
          if (!rootNode) {
            throw new Error('Root node not found');
          }

          let dynamicContentId = rootNode.data.nodes.find(nodeId => 
            currentState[nodeId].data.type === 'DynamicContent'
          );

          if (!dynamicContentId) {
            // Create DynamicContent node if it doesn't exist
            dynamicContentId = `DynamicContent_${Date.now()}`;
            const dynamicContentNode = {
            ...
            actions.add(dynamicContentNode, 'ROOT');
            console.log('Created new DynamicContent node:', dynamicContentId);
          }

          return dynamicContentId;
        };

        const dynamicContentId = findOrCreateDynamicContent();
