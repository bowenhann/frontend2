...
const AddNodeButton = () => {
  ...
  const findRootNodeId = (nodes) => {
    return Object.keys(nodes).find(id => !nodes[id].data.parent) || 'ROOT';
  };
  ...
  const addNewNode = () => {
    ...
    try {
      const rootNodeId = findRootNodeId(currentNodes);
      console.log('Detected root node ID:', rootNodeId);

      if (!currentNodes[rootNodeId]) {
      ...
      actions.addNodeTree(newNodeTree);

      console.log('New node added with ID:', newNodeId);
      console.log('Updated nodes:', JSON.stringify(query.getNodes(), null, 2));
    } catch (error) {
