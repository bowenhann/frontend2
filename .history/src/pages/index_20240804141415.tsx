...
const DynamicAdder = () => {
  ...
  const addDynamicText = () => {
    const node = query.createNode(<Element is={Text} text="Dynamically Added Text" />);
    actions.add(node, 'ROOT');
  };

  const addDynamicButton = () => {
    const node = query.createNode(<Element is={Button} text="Dynamic Button" />);
    actions.add(node, 'ROOT');
  };
...
export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Craft.js Drag and Drop with Dynamic Content Demo</h1>
      <Editor resolver={{ Text, Button, Container, DynamicAdder }}>
        <Frame>
          <Element is={Container} canvas>
            ...
            <Element is={DynamicAdder} /> {/* Add the DynamicAdder to the canvas */}
          </Element>
        </Frame>
      </Editor>
    </div>
