...
function renderComponents(componentsString) {
  const regex = /<(\w+)(\s[^>]*)?>([\s\S]*?)<\/\1>|<(\w+)(\s[^>]*)?\/>/g;
  const components = [];

  let match;
  while ((match = regex.exec(componentsString))) {
    const [fullMatch, openTagName, openAttributes, children, selfClosingTagName, selfClosingAttributes] = match;
    const tagName = openTagName || selfClosingTagName;
    const attributes = openAttributes || selfClosingAttributes || '';

    console.log(`Attempting to render component: ${tagName}`); // 调试日志
    ...
    if (Component) {
      ...
      let attributeMatch;
      while ((attributeMatch = attributeRegex.exec(attributes))) {
        const [, name, doubleQuotedValue, singleQuotedValue] = attributeMatch;
        const value = doubleQuotedValue || singleQuotedValue;

        if (name === 'className') {
          props[name] = `${props[name] || ''} ${value}`.trim();
        } else if (name === 'is') {
          const ResolvedComponent = componentMap[value];
          if (!ResolvedComponent) {
            console.error(`Component specified in 'is' attribute not found: ${value}`);
            continue;
          }
          props.is = ResolvedComponent;
        } else {
      ...
      if (tagName === 'Element') {
        ...
        components.push(
          React.createElement(Component, { key: components.length, ...props })
        );
      }
    } else {
      console.error(`Unknown component: ${tagName}`); // 错误日志
    }
  }
