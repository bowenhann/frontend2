...
const AddNodeButton = () => {
  ...
  const addNewNode = () => {
    console.log('Nodes before adding:', JSON.stringify(query.getNodes(), null, 2));

    const newNodeId = `Text_${Date.now()}`;
    
    // Directly modify the internal state
    store.actions.setProp(newNodeId, (prop) => {
      prop.type = { resolvedName: 'Text' };
      prop.props = { text: 'New Text' };
    ...
    store.actions.setProp('ROOT', (prop) => {
      prop.nodes = [...(prop.nodes || []), newNodeId];
    });

    // Force a re-render
    store.actions.setNodeEvent('selected', newNodeId);
