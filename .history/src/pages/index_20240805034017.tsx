...
// DynamicContent component remains the same
...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        console.log('Current state:', query.serialize()); // Log current state
        ...
        console.log('New state to be applied:', newState); // Log new state

        actions.deserialize(newState);
        
        console.log('State after deserialization:', query.serialize()); // Log state after update
      } catch (error) {
