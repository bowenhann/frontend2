  ...
  useEffect(() => {
    const addComponent = () => {
      ...
      try {
        ...
        // Use renderComponents to parse the string into React elements
        const parsedComponents = renderComponents(currentComponentString);

        // Wrap the parsed components in an Element with canvas property
        const wrappedComponent = (
          <Element canvas key={`stage-${stage}`}>
            {parsedComponents}
          </Element>
        );

        // Create a node tree from the wrapped component
