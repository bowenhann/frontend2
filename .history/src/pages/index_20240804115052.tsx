...
const componentConfigs = [
  ...
  },
  {
    type: ResizableComponent,
    props: { width: "20%", height: "50%" },
    children: <NodeButton>Button 3</NodeButton>
  },
  {
    type: ResizableComponent,
    props: { width: "95%", height: "35%" },
    children: (
      <NodeCard className="p-6 m-2">
        <NodeCardHeader>
          <NodeCardTitle className="bg-blue-500 text-white px-4 py-2 rounded">
...
const StreamingContent = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (renderedCount < componentConfigs.length) {
        const config = componentConfigs[renderedCount];
        const newNode = query.createNode(React.createElement(config.type, config.props, config.children));
  ...
  }, [actions, query, renderedCount]);

  return null;
};

StreamingContent.craft = {
  displayName: 'Streaming Content',
};

export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Streaming Render Demo</h1>
      <Editor
        resolver={{
          ResizableComponent,
          NodeButton,
