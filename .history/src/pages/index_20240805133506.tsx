		...
		switch (type) {
			...
			case 'button':
				if (buttonString) {
					...
					return (
						<Element id={`button_${id}`} is={Container} canvas={true}>
							{ButtonComponent}
						</Element>
					)
				}
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        let newContentId
        if (stage === 0) {
          // Create a new NewContent node for the first stage
          newContentId = `NewContent_Button_${Date.now()}`
          const newState = {
            ...parsedCurrentState,
            [dynamicContentId]: {
          ...
          // Update the existing NewContent node for subsequent stages
          newContentId = parsedCurrentState[dynamicContentId].nodes[parsedCurrentState[dynamicContentId].nodes.length - 1]
          const updatedButtonString = parsedCurrentState[newContentId].props.buttonString + buttonStrings[stage]
          actions.setProp(newContentId, (props) => props.buttonString = updatedButtonString)
        }
