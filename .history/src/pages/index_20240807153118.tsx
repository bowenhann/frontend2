...
import { Editor, Frame, Element, useEditor, Text } from '@craftjs/core';
...
const AddButton = () => {
  ...
  return (
    <button onClick={() => {
      actions.add(
        <CustomText text="New Text Added" />
      );
    }}>
      Add Text
    </button>
...
const App = () => {
  return (
    <Editor resolver={{CustomText}}>
      <Frame>
        <Element is={CustomText} text="Click the Add button to add more text" canvas />
        <Element is={AddButton} canvas={false} />
      </Frame>
    </Editor>
  );
};
