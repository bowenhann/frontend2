...
const DynamicContent = () => {
  ...
  React.useEffect(() => {
    const addComponents = () => {
      const newNode = query.createNode(React.createElement(SimpleComponent));
      actions.add(newNode, 'ROOT');

      // const buttonNode = query.createNode(React.createElement(Button));
      // actions.add(buttonNode, newNode.id);
    };
  ...
  return (
    <div className="p-4">
      ...
      <Editor resolver={{ SimpleComponent, Button, DynamicContent }}>
        <Frame>
          {/* <Element is={SimpleComponent} canvas> */}
            <Element is={DynamicContent} id="dynamic" canvas/>
          {/* </Element> */}
        </Frame>
      </Editor>
    </div>
