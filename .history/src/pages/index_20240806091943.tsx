    ...
    const timer = setInterval(() => {
      try {
        ...
        const findDynamicContent = (nodeId) => {
          ...
          // Safely check node properties
          const nodeType = node.data && node.data.type;
          const nodeName = node.data && node.data.name;
          const nodeDisplayName = node.data && node.data.displayName;

          if (nodeType === 'DynamicContent' || 
              nodeName === 'DynamicContent' || 
              nodeDisplayName === 'Dynamic Content') {
            return nodeId;
          }

          // Safely iterate over child nodes
          const childNodes = (node.data && node.data.nodes) || [];
