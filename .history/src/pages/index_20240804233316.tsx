...
const DynamicContent = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      actions.setProp(id, (props) => {
        props.isButton = true;
        props.text = "Dynamically Added Button";
      });
      console.log('Dynamic content updated to button');
    }, 2000);

    return () => clearTimeout(timer);
  }, [id, actions]);

  const { isButton, text } = query.node(id).get().props;

  if (isButton) {
    return <NodeButton text={text} />;
  }
  ...
  return (
    ...
    >
      Initial Content (Will change to button in 2 seconds)
    </div>
  );
};
...
export default function Home() {
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Craft.js Dynamic NodeButton Generation Demo</h1>
      <Editor resolver={{ Text, NodeButton, Container, DynamicContent }}>
        <Frame>
          <Element is={Container} canvas>
            <Element is={Text} text="Drag me around" />
            <Element is={NodeButton} text="Static Button" />
