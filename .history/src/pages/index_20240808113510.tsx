  ...
  const [newContentNodeId, setNewContentNodeId] = useState(null);
    ...
    const addOrUpdateNewContent = () => {
      if (!newContentNodeId) {
        ...
        // Use a setTimeout to ensure the node has been added before we try to find its ID
        setTimeout(() => {
          const addedNodeId = query.node((node) => node.data.type === NewContent).id;
          if (addedNodeId) {
            setNewContentNodeId(addedNodeId);
            console.log('Added new NewContent node with ID:', addedNodeId);
          } else {
            console.error('Failed to get ID of newly added NewContent node');
          }
        }, 0);
      } else {
