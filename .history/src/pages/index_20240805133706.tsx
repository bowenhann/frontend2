    ...
    const timer = setInterval(() => {
      try {
        ...
        if (stage === 0) {
          ...
          parsedCurrentState[dynamicContentId].nodes = [newContentId]
        } else {
          // Use the existing NewContent node for subsequent stages
          newContentId = parsedCurrentState[dynamicContentId].nodes[0]
        }

        // Update or create the NewContent node
        parsedCurrentState[newContentId] = {
          ...parsedCurrentState[newContentId],
          type: { resolvedName: 'NewContent' },
          props: {
            buttonString: buttonStrings.slice(0, stage + 1).join('\n'),
            id: newContentId
          },
