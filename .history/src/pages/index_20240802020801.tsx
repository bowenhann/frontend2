...
const DragLayer = () => {
  const { isDragging, currentNodeId } = useEditor((state) => ({
    isDragging: state.events.dragging,
    currentNodeId: state.events.selected,
  }));
...
const Container = ({ children }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '10px', margin: '10px', border: '1px solid black' }}>
      {children}
    </div>
  );
};
    ...
    >
      ...
      <Frame>
        <Element is={Container} canvas>
          <Element is={Text} text="Drag me to right panel!" />
          <Element is={Container} canvas>
...
const RightPanel = () => {
  ...
  return (
    ...
    >
      ...
      <Frame>
        <Element is={Container} canvas>
          <Element is={Text} text="Drop elements here!" />
        </Element>
      </Frame>
    </div>
  );
...
const App = () => {
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <h1>Craft.js with Cross-Panel Drag</h1>
      <Editor
