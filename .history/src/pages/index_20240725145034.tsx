...
function renderComponents(componentsString) {
  ...
  while ((match = regex.exec(componentsString))) {
    ...
    if (componentName === 'Element') {
      ...
      const childComponents = children ? renderComponents(children) : [];
      
      components.push({
        type: Element,
        props: {
          is: componentMap[props.is] || props.is,
          canvas: props.canvas === 'true',
          ...props,
  ...
  if (attributes) {
    ...
    while ((attributeMatch = attributeRegex.exec(attributes))) {
      ...
      if (name === 'is') {
        ...
        props[name] = jsValue ? eval(jsValue) : (doubleQuotedValue || singleQuotedValue);
      }
    }
  }
  return props;
...
export default function Home() {
	...
	const componentStructures = renderComponents(buttonString);
			...
			>
				<div className="flex flex-1 relative overflow-hidden">
                  ...
                  <NodeButton>Button 4</NodeButton> */}
									...
									{/* {components} */}
									<React.Fragment>
    {componentStructures.map((structure, index) => 
      React.createElement(structure.type, { ...structure.props, key: index })
    )}
  </React.Fragment>
