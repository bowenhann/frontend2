...
const ContentUpdater = () => {
  ...
  const buttonStrings = [
	...
	const findTargetNodeId = (nodes) => {
    const findNode = (nodeId) => {
      const node = nodes[nodeId];
      if (node.data.type.resolvedName === 'DynamicContent' || 
          node.data.type.resolvedName === 'Wrapper') {
        return nodeId;
      }
      for (const childId of node.data.nodes) {
        const result = findNode(childId);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const targetNodeId = findTargetNodeId(nodes);
        ...
        console.log('New node:', JSON.stringify(newNode, null, 2));
