...
const EditableText = ({ text, setText, color = '#000000' }) => {
...
const DynamicContainer = ({ items: initialItems }) => {
  ...
  const addNewItem = () => {
    setItems([...items, { text: 'New Item', color: '#000000' }]);
  };

  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '20px', border: '1px solid #ccc' }}>
      {items.map((item, index) => (
        <Element 
          ...
          text={item.text}
          color={item.color}
          setText={(text) => handleTextChange(text, index)}
        />
      ))}
      <button onClick={addNewItem} style={{ marginTop: '10px' }}>Add New Item</button>
    </div>
  );
};

DynamicContainer.craft = {
  props: {
    items: [
...
const App = () => {
  return (
    <div style={{ margin: '20px' }}>
      <h1>Dynamic Editable Content Demo</h1>
      <Editor resolver={{ DynamicContainer, EditableText }}>
        <Frame>
          <Element is={DynamicContainer} id="ROOT" canvas>
            <Element is={EditableText} id="dd" canvas>Editable Text</Element>
          </Element>
        </Frame>
      </Editor>
    </div>
