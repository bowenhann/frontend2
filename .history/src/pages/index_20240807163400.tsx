...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    // Add the initial container only once
    const initialNode = query.createNode({
      data: { type: Container }
    });
    actions.add(initialNode, 'ROOT');
    ...
    const timer = setInterval(() => {
      ...
      try {
        ...
        const containerNodeId = query.node('ROOT').childNodes()[0];

        if (containerNodeId) {
          // Create the new content
          const newContent = renderComponents(buttonStrings[stage]);

          // Parse the new content into a node
          const contentNode = query.parseReactElement(newContent).toNodeTree();

          // Remove existing children
          query.node(containerNodeId).childNodes().forEach(childId => {
            actions.delete(childId);
          });

          // Add new content
          actions.addNodeTree(contentNode, containerNodeId);
        }
