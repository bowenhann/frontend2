...
function parseAttributes(attributesString) {
  ...
  while ((match = regex.exec(attributesString)) !== null) {
    ...
    attributes[name] = jsValue || doubleQuotedValue || singleQuotedValue;
  }
  return attributes;
}
...
function parseComponentString(str) {
  ...
  while ((match = regex.exec(str)) !== null) {
    const [, openTag, openAttributes, children, selfClosingTag, selfClosingAttributes] = match;
    const tagName = openTag || selfClosingTag;
    const attributesString = openAttributes || selfClosingAttributes;

    const attributes = parseAttributes(attributesString);
    const parsedChildren = children ? parseComponentString(children) : [];

    if (tagName === 'Element') {
      const { is, ...restAttributes } = attributes;
      const Component = componentMap[is];
      if (!Component) {
        console.warn(`Component ${is} not found in componentMap`);
        continue;
      }
      components.push(
      ...
      if (!Component) {
        console.warn(`Component ${tagName} not found in componentMap`);
        continue;
      }
      components.push(React.createElement(Component, attributes, parsedChildren));
    }
  }
...
function CraftComponentsRenderer({ componentString }) {
  const { actions } = useEditor();

  React.useEffect(() => {
    const parsedComponents = parseComponentString(componentString);
    parsedComponents.forEach(component => {
      actions.add(component, 'ROOT');
    });
  }, [componentString, actions]);
			...
			>
				<div className="flex flex-1 relative overflow-hidden">
                  ...
                  <NodeButton>Button 4</NodeButton> */}
									...
									{/* {components} */}
									<CraftComponentsRenderer componentString={buttonString} />
