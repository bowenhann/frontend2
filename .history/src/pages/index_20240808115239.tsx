...
const ContentUpdater = () => {
  ...
  const [newContentNodeId, setNewContentNodeId] = useState(null);
    ...
    const timer = setInterval(() => {
      if (stage >= componentStrings.length - 1) {
        clearInterval(timer);
        return;
      }
      ...
      try {
        const componentString = componentStrings[stage];
        const ComponentToAdd = renderComponents(componentString);

        // Create a fresh node object
        const freshNode = {
          data: {
            type: ({ children }) => <div className="m-2">{children}</div>,
            props: {
