...
const Canvas = ({ children }) => {
  ...
  return (
    <div ref={connect} style={{ minHeight: '200px', border: '1px solid black', padding: '10px' }}>
      {children}
    </div>
  );
};
...
const AddButton = () => {
  ...
  return (
    <button onClick={() => {
      const canvasNodeId = query.getNodes().filter(node => node.data.type === Canvas.craft.displayName)[0].id;
      console.log('canvasNodeId:', canvasNodeId);
			actions.add(
        <Text text="New Text" />,
        canvasNodeId
      );
...
const App = () => (
  <div>
    <h1>CraftJS Add Demo</h1>
    <Editor resolver={{ Canvas, Text }}>
      <Frame>
        <Element is={Canvas} canvas>
          <Element is={Text} text="Initial Text" />
        </Element>
      </Frame>
      <AddButton />
    </Editor>
