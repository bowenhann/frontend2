...
const DynamicContent = ({ id, buttonStrings, stage }) => {
  const { connectors: { connect, drag } } = useNode();
  ...
  const renderContent = () => {
    ...
    const ButtonComponent = renderComponents(combinedString);
    return (
      <Element id={`dynamic_content_${stage}`} canvas>
        {ButtonComponent}
      </Element>
    );
  };
...
const ContentUpdater = () => {
  ...
  const buttonStrings = [
	...
	useEffect(() => {
    // First, add the container if it doesn't exist
    const containerNode = query.node('dynamic_content_container').get();
    if (!containerNode) {
      const freshNode = {
        data: {
          type: DynamicContent,
