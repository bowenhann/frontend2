...
Container.craft = { displayName: 'Container', isCanvas: true };
...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      console.log('Current nodes:', safeStringify(currentNodes));

      let rootNodeId = Object.keys(currentNodes).find(id => 
        currentNodes[id].data.type.resolvedName === 'Container' || 
        id === 'ROOT'
      );
      ...
      if (!rootNodeId) {
        console.log('Root container not found, creating a new one');
        rootNodeId = 'ROOT';
        currentNodes[rootNodeId] = {
          data: {
      ...
      const updatedNodes = {
        ...
        [rootNodeId]: {
          ...currentNodes[rootNodeId],
          data: {
            ...currentNodes[rootNodeId].data,
            nodes: [...(currentNodes[rootNodeId].data.nodes || []), newNodeId]
          }
        },
        [newNodeId]: {
          data: {
      ...
      console.log('Updated nodes before deserialize:', safeStringify(updatedNodes));

      actions.deserialize(updatedNodes);

      console.log('Nodes after deserialize:', safeStringify(query.getNodes()));
    }, 2000);
