...
const StaticComponent = () => {
  ...
  return (
    <div className="border border-gray-300 p-4 m-2">
      <h2 className="text-lg font-bold mb-2">Static Component</h2>
      <p className="mb-2">This is a static component</p>
    </div>
  );
};
...
const DynamicContent = () => {
  ...
  useEffect(() => {
    ...
    const addComponents = () => {
      ...
      try {
        const staticComponentNode = query.createNode(React.createElement(StaticComponent));
        actions.add(staticComponentNode, 'ROOT');
        console.log('StaticComponent added successfully');
      } catch (error) {
        console.error('Error adding StaticComponent:', error);
      }
    };
  ...
  console.log('DynamicContent rendered');
  return <div>Dynamic Content Placeholder</div>;
};

StaticComponent.craft = {
  displayName: 'Static Component',
};
...
export default function Home() {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Craft.js Debug Demo</h1>
      <Editor resolver={{ StaticComponent, DynamicContent }}>
        <Frame>
          <Element is={StaticComponent} canvas>
