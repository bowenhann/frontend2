...
const ContentUpdater = () => {
  ...
  const [newContentNodeId, setNewContentNodeId] = useState(null);
	...
	useEffect(() => {
    const addOrUpdateNewContent = () => {
      if (!newContentNodeId) {
				console.log('Adding new content node');
        // If we don't have a newContentNodeId, create a new node
        const freshNode = {
          data: {
            type: NewContent,
            props: {
        ...
        const newNodeId = Object.keys(rootNode.data.nodes).find(id => 
          rootNode.data.nodes[id].data.type === NewContent
        );
        
        if (newNodeId) {
					
          setNewContentNodeId(newNodeId);
          console.log('Added new NewContent node with ID:', newNodeId);
        } else {
          console.error('Failed to find the newly added NewContent node');
        }
      } else {
				console.log('Updating existing content node');
        // If we have a newContentNodeId, update the existing node
        actions.setProp(newContentNodeId, (props) => {
          props.stage = stage;
        });
        console.log('Updated existing NewContent node with ID:', newContentNodeId);
      }
    };
  ...
  }, [actions, query, stage, buttonStrings, newContentNodeId]);
