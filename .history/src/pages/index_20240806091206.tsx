  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        if (!dynamicContentId) {
          console.error('DynamicContent node not found. Current state:', parsedCurrentState)
        ...
        // 确保父节点存在且有正确的结构
        if (!parsedCurrentState[dynamicContentId] || !parsedCurrentState[dynamicContentId].nodes) {
          console.error('Parent node is not properly structured:', parsedCurrentState[dynamicContentId])
          return
        }
        ...
        // 使用 addNodeTree 添加新节点
        actions.addNodeTree(newContentNode, dynamicContentId)

        setStage(stage + 1)
      } catch (error) {
        console.error('Error updating content:', error)
        console.error('Current editor state:', query.serialize())
        clearInterval(timer)
      }
    }, 2000)
