  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        console.log('All nodes (simplified):', safeStringify(Object.values(nodes).map(getSimplifiedNode)));

        let targetNodeId = Object.keys(nodes).find(nodeId => 
          nodes[nodeId].data.name === 'DynamicContent' || 
          nodes[nodeId].data.name === 'Wrapper' ||
          nodes[nodeId].data.name === 'Frame'
        ) || 'ROOT';

        console.log('Target node ID:', targetNodeId);

        // Check if the target node exists and has a nodes array
        const targetNode = nodes[targetNodeId];
        if (!targetNode) {
          throw new Error(`Target node ${targetNodeId} not found`);
        }
        if (!Array.isArray(targetNode.data.nodes)) {
          console.warn(`Target node ${targetNodeId} does not have a nodes array. Initializing it.`);
          actions.setCustom(targetNodeId, {
            ...targetNode.data.custom,
            nodes: []
          });
        }

        const newNodeId = `NewContent_${Date.now()}`;
        ...
        const newNodeTree = {
          [newNodeId]: {
            ...
            nodes: [containerId],
            linkedNodes: {
              'dynamic_content_container': containerId
            }
          },
          [containerId]: {
        ...
        console.log('New node tree:', safeStringify(newNodeTree));

        // Add the new node tree
        actions.addNodeTree(newNodeTree, targetNodeId);

        // Update the parent's nodes array
        const updatedTargetNode = query.node(targetNodeId).get();
        if (updatedTargetNode) {
          const updatedNodes = [...(updatedTargetNode.data.nodes || []), newNodeId];
          console.log(`Updating ${targetNodeId} nodes:`, updatedNodes);
          actions.setCustom(targetNodeId, {
            ...updatedTargetNode.data.custom,
            nodes: updatedNodes
          });
        } else {
          console.error(`Failed to get updated target node ${targetNodeId}`);
        }

        // Check if the new node was actually added
        setTimeout(() => {
          const updatedNodes = query.getNodes();
          console.log('Updated nodes after adding (simplified):', 
            safeStringify(Object.values(updatedNodes).map(getSimplifiedNode))
          );
          if (updatedNodes[newNodeId]) {
            console.log('New node successfully added');
          } else {
