...
const ContentUpdater = () => {
  ...
  const components = [
    {
      ...
      children: [
        {
          type: 'div',
          props: { className: "bg-gray-800 text-white p-4" },
          children: [
            {
              type: 'h1',
  ...
  const renderComponentToString = (component) => {
    ...
    const childrenString = Array.isArray(children)
      ? children.map(renderComponentToString).join('')
      : children || ''
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      try {
        ...
        const newContentId = `NewContent_Component_${Date.now()}`
        ...
        const newState = {
          ...
          [newContentId]: {
            ...
            props: {
              type: 'button',
              buttonString: renderComponentToString(components[componentIndex]),
              id: newContentId
            },
            displayName: `New Content Component ${componentIndex + 1}`,
            custom: {},
            hidden: false,
            nodes: [],
            linkedNodes: {},
        ...
        console.log(`Rendering component ${componentIndex + 1}`)
  ...
  }, [actions, query, componentIndex, components])
