...
const Container = ({ background, padding }) => {
  ...
  return (
    <div 
      ...
      style={{ background, padding: `${padding}px`, margin: '5px', border: '1px solid black' }}
    >
      <h2>Container</h2>
      <button onClick={() => {
        const currentNode = query.node(node => node.data.props).get();
        actions.add(
          query.createNode(Element.create(Container, {
            background: currentNode.data.props.background,
            padding: currentNode.data.props.padding,
          }))
        );
      }}>
        复制容器
      ...
      <Editor resolver={{ Container }}>
        <Frame>
          <Element is={Container} canvas>
            <Container background="#e0e0e0" padding={10} />
          </Element>
        </Frame>
      </Editor>
    </div>
