...
const NodeButton = ({ className, children }) => {
  const { connectors: { connect, drag } } = useNode();
  return (
    <button 
      ref={ref => connect(drag(ref))}
...
const DynamicContent = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const newContent = (
        <Element is={NodeButton} canvas className="bg-blue-500 text-white px-4 py-2 rounded">
...
export default function Home() {
  return (
    <div className="p-4">
      ...
      <Editor resolver={{ Text, Button, Container, DynamicContent, NodeButton }}>
        <Frame>
