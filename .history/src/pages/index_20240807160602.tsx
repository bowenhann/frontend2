...
const ContentUpdater = () => {
  ...
  useEffect(() => {
    // Add the initial container if it doesn't exist
    const initialSetup = () => {
      const rootNode = query.node('ROOT').get();
      if (!rootNode.data.nodes.includes('dynamic_content_container')) {
        const initialContainer = query.parseReactElement(
          <Element
            canvas
            is={NewContent}
        ...
        actions.addNodeTree(initialContainer, 'ROOT');
      }
    };
    ...
    const timer = setInterval(() => {
      ...
      try {
        // Update the existing container instead of adding a new one
