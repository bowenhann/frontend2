...
const Draggable = ({ children }) => {
  ...
  return (
    <div ref={(ref) => connect(drag(ref))} style={{ padding: '10px', margin: '5px', border: '1px dashed gray' }}>
      {children}
    </div>
  );
};
...
const NewContent = ({ content, type }) => {
  const renderContent = () => {
    switch (type) {
      case 'text':
        return <div style={{ background: 'pink', padding: '10px' }}>{content}</div>;
      case 'button':
        return <NodeButton>{content}</NodeButton>;
      default:
...
// DynamicContent component (now a container)
const DynamicContent = ({ children }) => {
  return (
    <Draggable>
      <div style={{ background: 'lightyellow', padding: '20px', minHeight: '100px' }}>
...
const Container = ({ children }) => {
  return (
    <Draggable>
      <div style={{ background: 'lightgreen', padding: '20px', minHeight: '100px' }}>
        {children}
      </div>
    </Draggable>
  );
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        ...
        // Function to recursively find the DynamicContent node
        ...
        // Create a new NewContent node
        const newContentId = `NewContent_${Date.now()}`;
        const newState = {
          ...parsedCurrentState,
          [dynamicContentId]: {
            ...parsedCurrentState[dynamicContentId],
            nodes: [...(parsedCurrentState[dynamicContentId].nodes || []), newContentId],
          },
          [newContentId]: {
            type: { resolvedName: 'NewContent' },
            isCanvas: false,
            props: { content: 'New Content (Drag me!)' },
            displayName: 'New Content',
            custom: {},
            hidden: false,
            nodes: [],
        ...
        console.log('Updating content, adding NewContent to DynamicContent');
...
const App = () => {
  return (
    <div>
      ...
      <Editor resolver={{ DynamicContent, TextBox, Container, NewContent }}>
        <Frame>
