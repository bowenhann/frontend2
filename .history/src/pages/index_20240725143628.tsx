...
function renderComponents(componentsString) {
  ...
  let match;
  while ((match = regex.exec(componentsString))) {
    const [fullMatch, openTag, openAttributes, children, selfClosingTag, selfClosingAttributes] = match;
    
    const tagName = openTag || selfClosingTag;
    const attributes = parseAttributes(openAttributes || selfClosingAttributes);
    
    let Component;
    if (tagName === 'Element') {
      Component = Element;
      if (attributes.is && typeof attributes.is === 'string') {
        attributes.is = componentMap[attributes.is] || attributes.is;
      }
    } else {
      Component = componentMap[tagName] || tagName;
    }
...
function parseAttributes(attributeString) {
  ...
  while ((match = regex.exec(attributeString))) {
    ...
    if (name === 'className') {
      attributes[name] = `${attributes[name] || ''} ${value}`.trim();
    } else if (name === 'canvas' || name === 'draggable') {
      attributes[name] = value !== 'false'; // Convert to boolean
    } else {
      try {
        // Attempt to parse as JSON (for objects, arrays, booleans, null)
