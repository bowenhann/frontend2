...
// Craft.js 组件包装器
const TextComponent = ({text, fontSize}) => {
  return (
    <Element canvas>
...
const AddNodeDemo = () => {
  ...
  const addTextNode = () => {
    actions.add(
      <TextComponent text="新的文本节点" fontSize={16} />,
      'ROOT'
    );
  };
  ...
  return (
    <div>
      ...
      <button onClick={addButtonNode}>添加按钮节点</button>
    </div>
  );
};
...
const App = () => {
  return (
    <Editor resolver={{Text: TextComponent, Button: ButtonComponent}}>
      <Frame>
        <Element canvas is={AddNodeDemo} />
      </Frame>
    </Editor>
  );
};
