...
const ContentUpdater = () => {
  ...
  const buttonStrings = [
	...
	useEffect(() => {
    // Create the initial container only once
    ...
    const containerNode = query.parseFreshNode(initialContainer).toNode();
    actions.add(containerNode, 'ROOT');
    ...
    const timer = setInterval(() => {
      ...
      try {
        // Find the existing NewContent node
        const newContentNode = query.node((node) => node.data.type === NewContent).get();

        if (newContentNode) {
          // Update the existing node's props
          actions.setProp(newContentNode.id, (props) => {
            props.stage = stage + 1;
          });
        } else {
          console.error('NewContent node not found');
        }
...
// Update NewContent to handle stage updates
const NewContent = ({ buttonStrings, stage }) => {
  const { connectors: { connect, drag } } = useNode();
