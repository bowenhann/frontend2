...
const ContentUpdater = () => {
  ...
  const buttonStrings = [
	...
	useEffect(() => {
    // First, add the initial NewContent if it doesn't exist
    const initializeNewContent = () => {
      ...
      if (!existingNewContent) {
        ...
        actions.add(node, 'ROOT', actions.getNodes().length);
    ...
    const timer = setInterval(() => {
      ...
      try {
        // Update the existing NewContent node
        actions.setCustom('NEW_CONTENT', (node) => {
          node.data.props.stage = stage + 1;
        });
