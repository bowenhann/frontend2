...
const StaticText = ({ text }) => <p>{text}</p>;
...
const ResizableContainer = ({ children }) => {
  ...
  return (
    <div 
      ...
      style={{ width: '100%', height: '300px', border: '2px solid #000', margin: '10px 0' }}
    >
      <PanelGroup direction="horizontal">
        <Panel minSize={20}>
          <div style={{ padding: '10px', height: '100%', background: '#f0f0f0' }}>
            <StaticText text="Left Panel (Static)" />
            {children[0]}
          </div>
        </Panel>
        <PanelResizeHandle />
        <Panel minSize={20}>
          <div style={{ padding: '10px', height: '100%', background: '#e0e0e0' }}>
            <StaticText text="Right Panel (Static)" />
            {children[1]}
          </div>
        </Panel>
      </PanelGroup>
...
const App = () => {
  return (
    <div style={{ margin: '50px' }}>
      ...
      <Editor resolver={{ Text, StaticText, ResizableContainer }}>
        <Frame>
          <Element is={ResizableContainer} canvas>
            <Element is={Text} id="left-text" text="Left panel (draggable)" />
            <Element is={Text} id="right-text" text="Right panel (draggable)" />
          </Element>
          <Element is={ResizableContainer} canvas>
            <Element is={Text} id="left-text-2" text="Another left panel (draggable)" />
            <Element is={Text} id="right-text-2" text="Another right panel (draggable)" />
          </Element>
        </Frame>
      </Editor>
    </div>
