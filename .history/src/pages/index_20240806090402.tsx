  ...
  const buttonStrings = [
    ...
    const timer = setInterval(() => {
      try {
        ...
        console.log('Dynamic Content ID:', dynamicContentId)
        console.log('Current State:', parsedCurrentState)
        ...
        // Check if dynamicContentId exists in the current state
        if (!parsedCurrentState[dynamicContentId]) {
          throw new Error(`Dynamic Content ID ${dynamicContentId} not found in current state`)
        }

        // Use addNodeTree instead of deserialize
        actions.addNodeTree(newContentNode, dynamicContentId)

        console.log('Node tree added successfully')
